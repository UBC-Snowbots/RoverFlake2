...
void CBSArmInterface::arm_panel_callback(const rover_msgs::msg::ArmPanel::SharedPtr msg){
   ...
   //  arm_ik_pub->publish(ik_msg);
    ...
    cmd_msg.velocities[0] = (static_cast<float>(msg->left.x) - 50)/100 * max_joysticks_output_speed_deg[0]*2 * multiply;
     cmd_msg.velocities[1] = (static_cast<float>(msg->left.y) - 50)/100 * max_joysticks_output_speed_deg[1]*2 * multiply;
     cmd_msg.velocities[2] = (static_cast<float>(msg->right.y) - 50)/100 * max_joysticks_output_speed_deg[2]*2*-1 * multiply;
     cmd_msg.velocities[3] = (static_cast<float>(msg->left.z) - 50)/100 * max_joysticks_output_speed_deg[3]*2 * multiply;
     cmd_msg.velocities[4] = (static_cast<float>(msg->right.x) - 50)/100 * max_joysticks_output_speed_deg[4]*2 * multiply;
     cmd_msg.velocities[5] = (static_cast<float>(msg->right.z) - 50)/100 * max_joysticks_output_speed_deg[5]*2 * -1 * multiply;
     
     if(msg->left.button && msg->right.button){
        
        cmd_msg.velocities[6] = 0;
    cmd_msg.end_effector = 0;
    }else{
    if(msg->left.button){
      cmd_msg.velocities[6] = -28.8 * multiply_ee;
...
//     ik = true;
//  }
//! OFF IS ON RN CUS THING IS MOUNTED UPSIDE DOWN

if(!msg->switches[2])
{
// RCLCPP_INFO(this->get_logger(), "PANEL A mul 2");
...
if(!msg->switches[1])
{
   ...
   multiply_ee = 1.0;
// RCLCPP_INFO(this->get_logger(), "PANEL A mul 1");
