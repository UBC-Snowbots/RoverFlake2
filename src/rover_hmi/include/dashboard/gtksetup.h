            ...
            for (int i = 0; i < NUM_MONITORED_SYSTEMS; i++) {
              ...
              builder->get_widget("subsys_" + index + "_run_button", system_health.system[i].run_button);
                ...
                for(int i = 0; i < NUM_MONITORED_SYSTEMS; i++){
                    ...
                    system_health.system[i].kill_button->signal_pressed().connect(sigc::bind(sigc::mem_fun(*this, &DashboardHMINode::subsystemRequest), monitored_system_names[i], KILL, COMPUTER_GLOBAL));
                    system_health.system[i].run_button->signal_pressed().connect(sigc::bind(sigc::mem_fun(*this, &DashboardHMINode::subsystemRequest), monitored_system_names[i], RUN, COMPUTER_GLOBAL));
