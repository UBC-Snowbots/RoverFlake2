...
(`tile_map` is built from patched source: the `ros-humble-tile-map` 2.6.5 deb
ships a lib whose Q_OBJECT headers were never moc'd, so mapviz dies with
`undefined symbol: _ZTVN8tile_map12StadiaSourceE`. The vendored mapviz 2.6.5
patches its CMake — moc for QT_HEADERS, link OpenGL::GLU — and builds only
`tile_map`; siblings carry COLCON_IGNORE since debs cover them.)
