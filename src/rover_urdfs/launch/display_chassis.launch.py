...
def generate_launch_description():
   ...
   # servo_yaml = load_yaml("rover_urdfs", "config/rover_servo_params.yaml")
    ...
    # RViz
    rviz_config_file = (
        get_package_share_directory("rover_urdfs") + "/config/full_urdf_display.rviz"
    )

    urdf_file_path = (
        get_package_share_directory("rover_urdfs") + "/urdf/Full_Assembly.urdf"
    )
    ...
    full_urdf_publisher = Node(
            ...
            output='screen',
            parameters=[{'robot_description': xacro_to_urdf("rover_urdfs", urdf_file_path) }]
        )
