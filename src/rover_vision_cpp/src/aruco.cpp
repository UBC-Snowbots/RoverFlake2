#include "rover_vision_cpp/aruco.h"
    ...
    : rclcpp::Node("marker_qr_detection", options) 
{
    ...
    topic_to_subscribe_to = this->get_parameter("camera_topic").as_string();

    // Setup Publisher
    my_publisher = this->create_publisher<std_msgs::msg::String>("identified", 10);

    RCLCPP_INFO(this->get_logger(), "Subscribing to topic: %s", topic_to_subscribe_to.c_str());
    if (draw_markers) {
